            ...
            <Card className="border-0 shadow-sm overflow-hidden bg-white dark:bg-gray-900">
              <CardContent className="p-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    <ProgramIcon program={requestDetails.programType} size="lg" />
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 className="text-xl font-bold text-foreground">{requestDetails.requestNumber}</h2>
                        <Badge variant="outline" className="text-xs font-semibold py-0.5">
                          {getStageLabel(requestDetails.currentStage, requestDetails.requestTrack)}
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground mt-1.5 flex items-center gap-1.5">
                        <Building2 className="w-4 h-4" />
                        {requestDetails.mosque?.name || "—"} ({requestDetails.mosque?.city || "—"})
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        link.href = url;
                        link.download = `قالب_جدول_الكميات_${requestDetails.requestNumber}.csv`;
                        link.click();
