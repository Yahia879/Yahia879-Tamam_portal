      ...
      <div className="space-y-6 animate-fade-in pb-20" dir="rtl">
        ...
        {step === 1 ? (
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            <Card className="border-0 shadow-xl shadow-emerald-950/5 ring-1 ring-emerald-100/50 bg-white/80 backdrop-blur-md">
              <CardHeader className="text-right border-b border-emerald-50/50 pb-4">
                <CardTitle className="flex items-center gap-2 justify-start flex-row-reverse text-right text-emerald-800 text-lg">
              ...
              <CardContent className="space-y-6 pt-6 text-right">
                <div className="space-y-2 text-right">
                  <Label className="text-right text-xs font-bold text-slate-700">المشروع *</Label>
                  <Select
                ...
                {formData.projectId > 0 && (
                  <div className="space-y-4 text-right animate-slide-up">
                    <div className="space-y-2 text-right">
                      <Label className="text-right text-xs font-bold text-slate-700">تقرير الإنجاز المرتبط *</Label>
                      <Select
                    ...
                    {selectedReport && (
                      <div className="space-y-4 animate-slide-up">
                        {/* تنبيه إلزامي للاطلاع */}
                        <div className="p-4 rounded-xl border border-amber-200/60 bg-amber-50/50 text-amber-900 flex items-start gap-3 text-right">
                          <AlertCircle className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" />
                          <div className="space-y-1">
                            <p className="text-xs font-bold text-amber-800">ملاحظة تنظيمية هامة</p>
                            <p className="text-[11px] leading-relaxed text-amber-700/90 font-medium">
                              يجب الاطلاع على تقرير الإنجاز المرفق وتدقيق البيانات المالية ومطابقتها قبل استكمال الإجراءات المتبقية للتأكد من توافق الدفعات مع المنجز الفعلي.
                            </p>
                          </div>
                        </div>

                        {/* معلومات الربط بنجاح ومعاينة التقرير */}
                        <div className="p-4 rounded-xl border border-emerald-200/60 bg-emerald-50/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-right">
                          <div className="space-y-1">
                            <p className="text-xs font-bold text-emerald-800 flex items-center gap-1.5 justify-start flex-row-reverse">
                              <CheckCircle className="w-4 h-4 text-emerald-600" />
                              تم ربط تقرير الإنجاز بنجاح
                            </p>
                            <p className="text-[11px] font-semibold text-slate-600">
                              نسبة الإنجاز الفعلي بالتقرير: <strong className="text-emerald-700">{selectedReport.actualProgress}%</strong> (المخطط: {selectedReport.plannedProgress}%)
                            </p>
                          </div>
                          <Button
                            type="button"
                            variant="outline"
                            size="sm"
                            className="text-emerald-700 border-emerald-300/60 bg-background hover:bg-emerald-50 font-bold h-9 text-xs"
              ...
              <CardFooter className="border-t border-emerald-50/50 pt-4 flex justify-end gap-2">
                <Button
                  onClick={() => setStep(2)}
                  disabled={!selectedReportId}
                  className="gradient-primary text-white font-bold px-6 h-10"
                >
                  التالي
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6 text-right">
              <Card className="border-0 shadow-xl shadow-emerald-950/5 ring-1 ring-emerald-100/50">
                <CardHeader className="text-right border-b border-emerald-50/50 pb-4">
                  <CardTitle className="flex items-center gap-2 justify-start flex-row-reverse text-right text-emerald-800 text-lg">
                    <FileText className="h-5 w-5 text-emerald-600" />
                ...
                <CardContent className="space-y-4 pt-6 text-right">
                  ...
                  {paymentInfo && (
                    <div className="p-4 rounded-xl border border-emerald-200/50 bg-emerald-50/20 text-right space-y-2">
                      <Label className="text-xs font-bold text-emerald-800 block">الدفعة المتفق عليها (مجدولة في العقد) *</Label>
                      <div className="flex items-baseline gap-2 justify-start flex-row-reverse">
                        <span className="text-2xl font-bold text-emerald-700">{parseFloat(paymentInfo.amount || "0").toLocaleString()}</span>
                        <span className="text-xs text-muted-foreground font-bold">ريال سعودي</span>
                      </div>
                      <p className="text-[10px] text-slate-500 font-medium">الوصف المعتمد: {paymentInfo.description}</p>
                    </div>
                  )}

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2 text-right">
                      <Label className="text-right text-xs font-bold text-slate-700">التاريخ الميلادي *</Label>
                      <Input
                        type="date"
                    ...
                    <div className="space-y-2 text-right">
                      <Label className="text-right text-xs font-bold text-slate-700">نسبة الإنجاز المرتبطة بالطلب (%) *</Label>
                      <Input
                        type="number"
                        min="1"
                        max="100"
                  ...
                  <div className="space-y-2 text-right">
                    <Label className="text-right text-xs font-bold text-slate-700">عنوان طلب الصرف *</Label>
                    <Input
                  ...
                  <div className="space-y-2 text-right">
                    <Label className="text-right text-xs font-bold text-slate-700">وصف الأعمال والمنجزات الفعلية *</Label>
                    <Textarea
                ...
                <CardHeader className="text-right border-b border-emerald-50/50 pb-4">
                  <div className="flex items-center justify-between flex-row-reverse">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={addSupplier}
                      className="text-emerald-700 border-emerald-300 hover:bg-emerald-50 font-bold h-8 text-xs"
                    >
                ...
                <CardContent className="pt-6">
                  <div className="space-y-6">
                    {suppliers.map((supplier, index) => (
                      <div key={supplier.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 relative space-y-4 text-right animate-slide-up">
                        {suppliers.length > 1 && (
                          <Button
                            type="button"
                            variant="ghost"
                            size="icon"
                            className="absolute top-2 left-2 text-rose-500 hover:text-rose-700 hover:bg-rose-50 h-8 w-8 rounded-full"
                        ...
                        <div className="flex items-center gap-2 text-xs font-bold text-emerald-800 justify-start flex-row-reverse">
              ...
              {/* Navigation Actions */}
              <div className="flex items-center justify-between border-t border-emerald-100/50 pt-4 flex-row-reverse">
                <Button
                  onClick={handleSubmit}
                  disabled={createMutation.isPending}
                  className="gradient-primary text-white font-bold px-8 h-11"
                >
                ...
                <Button
                  variant="outline"
                  onClick={() => setStep(1)}
                  className="text-emerald-700 border-emerald-200 hover:bg-emerald-50 font-bold px-6 h-11 text-xs"
                >
            ...
            {/* Sidebar Summary */}
            <div className="space-y-6">
              <Card className="border-0 shadow-xl shadow-emerald-950/5 ring-1 ring-emerald-100/50 sticky top-6">
                <CardHeader className="text-right border-b border-emerald-50/50 pb-4">
                  <CardTitle className="text-emerald-800 text-base">ملخص الدفعة والتقرير</CardTitle>
                ...
                <CardContent className="pt-6 space-y-4 text-right">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold text-slate-800 leading-snug">{projectDetails?.name || "المشروع المحدد"}</p>
                  </div>
                  
                  <Separator className="bg-emerald-50/50" />
                  
                  <div className="space-y-1">
                    <p className="text-[10px] text-muted-foreground font-semibold">رقم تقرير الإنجاز</p>
                    <p className="text-xs font-bold text-slate-800">{selectedReport?.reportNumber || "لا يوجد"}</p>
                  </div>
                  ...
                  <div className="space-y-1 p-3 bg-emerald-50/30 rounded-lg">
                    <p className="text-[10px] text-emerald-800 font-bold">الدفعة الفعلية التي سوف تصرف</p>
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between flex-row-reverse text-xs">
                          <span className="text-muted-foreground font-medium">مبلغ العقد الإجمالي</span>
                          <span className="font-bold text-slate-700">{contractAmount.toLocaleString()} ريال</span>
                        </div>
                        <div className="flex justify-between flex-row-reverse text-xs">
                          <span className="text-rose-600 font-medium">إجمالي المصروف سابقاً</span>
                          <span className="font-bold text-rose-600">{totalPaymentsSum.toLocaleString()} ريال</span>
                        </div>
                        <div className="flex justify-between flex-row-reverse text-xs pt-1 border-t border-dashed">
                          <span className="text-emerald-700 font-bold">المتبقي للصرف</span>
                          <span className="font-black text-emerald-700">{remainingAmount.toLocaleString()} ريال</span>
                        </div>
                      </div>
                    </>
                  )}
