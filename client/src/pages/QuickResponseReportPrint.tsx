...
export default function QuickResponseReportPrint() {
  const { id } = useParams();
  const [, setLocation] = useLocation();
  const { user: currentUser } = useAuth();
  const requestId = parseInt(id || "0");

  useDocumentTitle(requestId ? `طباعة تقرير الاستجابة السريعة #${requestId}` : "تقرير الاستجابة السريعة");
  ...
  if (!request) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 gap-4 p-4" dir="rtl">
        ...
        <p className="text-slate-500 text-sm">لم يتم العثور على بيانات الطلب أو تقرير الاستجابة السريعة.</p>
            ...
            <div className="relative z-10 space-y-4">
              ...
              <div className="section-block space-y-2">
                <h3 
                  ...
                  style={{ backgroundColor: '#d4a574', color: '#5d4037' }}
                >
                  3. تفاصيل الأعمال المنفذة والملاحظات:
                </h3>
              ...
              {reportPhotos.length > 0 && (
                <div className="section-block">
                  <h3 
                    ...
                    style={{ backgroundColor: '#d4a574', color: '#5d4037' }}
                  >
                    4. الصور التوثيقية للأعمال المنفذة ({reportPhotos.length} صور):
                  </h3>
