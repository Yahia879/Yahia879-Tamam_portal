...
async function main() {
  ...
  const res = await caller.sedanaExecution.getVirtualInventory({ requestId: 99 });
  console.log('--- REQ 99 INVENTORY ITEMS ---');
  ...
  console.log('--- REQ 99 OUTBOUND ORDERS ---', res.outboundOrders.length);
  res.outboundOrders.forEach((o: any) => {
    console.log(o.orderNumber, o.status, o.periodLabel);
