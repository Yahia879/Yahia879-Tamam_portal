...
async function main() {
  console.log('🚀 Starting Full Database Synchronization & Update...\n');
  ...
  try {
    ...
    for (const m of defaultModules) {
      ...
      if (existing.length === 0) {
        ...
        await connection.query(
          `INSERT INTO modules (id, nameAr, nameEn, icon, displayOrder, isActive)
           VALUES (?, ?, ?, ?, ?, ?)`,
    ...
    console.log('\n🎉 Database Full Synchronization completed successfully!');
  } catch (error) {
